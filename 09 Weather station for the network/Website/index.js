...
function showChart(result = null) {
  ...
  Chartist.Line('.ct-chart-td', setSeries(mas, iDay, dDay, 't10', 'st10'), options);
  Chartist.Line('.ct-chart-tm', setSeries(mas, iMonth, dMonth, 't10', 'st10'), options);
  Chartist.Line('.ct-chart-ty', setSeries(mas, iYear, dYear, 't10', 'st10'), options);
  
  options.low = 0;
  options.high = 100;
  // options.high = 100;
  
  Chartist.Line('.ct-chart-hd', setSeries(mas, iDay, dDay, 'h'), options);
  Chartist.Line('.ct-chart-hm', setSeries(mas, iMonth, dMonth, 'h'), options);
  Chartist.Line('.ct-chart-hy', setSeries(mas, iYear, dYear, 'h'), options);
...
setInterval(function() {
  ...
  .then(result => {
    ...
    if ( obj['t'] !== undefined ) {
      ...
      elem.innerHTML = obj['t10'].toFixed(1) + " C°";
    }
               
    if ( obj['h'] !== undefined ) {
      let elem = document.getElementById("hum");
      elem.innerHTML = obj['h'].toFixed(1) + " %";
    }
    

    if ( obj['id'] !== undefined ) {
      let elem = document.getElementById("id");
      elem.innerHTML = obj['id'].toString(16);
    }
