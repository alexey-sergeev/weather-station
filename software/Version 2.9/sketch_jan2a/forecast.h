...
class Forecast { 
  public: 
    ...
    void init() {
      ...
      String appid = FORECAST_APPID;
      String city = FORECAST_CITY;

      #ifdef SET_SETTINGS
      
        // String s;
        // s = storage.readFile(PATH_CONF_FORECAST);
        ...
        // if ( row.parse() ) a = row.trim().toString() ;
        // if ( a != "" ) appid = a;
        ...
        appid = settings.getForecast(0);
        String c = settings.getForecast(1);
        c.replace(" ", "+");
        if ( c != "" ) city = c;

      #endif

      _urlWeather = WEATHER_URL;
      _urlWeather += "?q=";
      _urlWeather += city;
      _urlWeather += "&cnt=7&APPID=";
      _urlWeather += appid;

      _urlForecast = FORECAST_URL;
      _urlForecast += "?q=";
      _urlForecast += city;
      _urlForecast += "&cnt=7&APPID=";
      _urlForecast += appid;
  ...
  private:
    ...
    String _getForecast(char wf = 'w') {
      ...
      // char urlWeather[] = WEATHER_URL "?q=" FORECAST_CITY "&cnt=7&APPID=" FORECAST_APPID;
      // char urlForecast[] = FORECAST_URL "?q=" FORECAST_CITY "&cnt=7&APPID=" FORECAST_APPID;
      // pln(_urlWeather);
      // pln(_urlForecast);

      if ( h.begin(client, ( wf == 'w' ) ? _urlWeather : _urlForecast) ) {
